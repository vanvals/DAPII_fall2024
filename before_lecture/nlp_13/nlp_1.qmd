...
- Converts words to their base form (e.g., texts $\to$ text, coding $\to$ code, fastest $\to$ fast)
...
There is a version incompatibility issue between `spaCy` and `numpy` (**[details](https://stackoverflow.com/questions/78634235/numpy-dtype-size-changed-may-indicate-binary-incompatibility-expected-96-from)**). It can usually be addressed by jointly installing `spaCy` and `numpy` so that `pip` ensures you get compatible versions. Some students may need to create a new virtual environment first (**[details](https://code.visualstudio.com/docs/python/environments)**)
...
![](pictures/flight_example.png)

Source: Speech and Language Processing textbook by Jurasfky and Martin. **[Link](https://web.stanford.edu/~jurafsky/slp3/19.pdf)**

## Dependency Grammars II

![](pictures/flight_tree.png){width=80%}
